    ...
    try:
        ...
        if not str(message.forward_from.id) != "93372553":
            ...
            try:
                ...
                await msg.edit_text(f"<b>𝚂𝚞𝚌𝚌𝚎𝚜𝚏𝚞𝚕𝚕𝚢 𝙲𝚕𝚘𝚗𝚎𝚍 𝚢𝚘𝚞𝚛 @{bot.username} .\n\n⚠️ <u>𝙳𝚘 𝙽𝚘𝚝 𝚂𝚎𝚗𝚍 𝚃𝚘 𝙰𝚗𝚢 𝙾𝚗𝚎</u> 𝚃𝚑𝚎 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚆𝚒𝚝𝚑 <u>𝚃𝚑𝚎 𝚃𝚘𝚔𝚎𝚗</u> 𝙾𝚏 𝚃𝚑𝚎 𝙱𝚘𝚝, 𝚆𝚑𝚘 𝙷𝚊𝚜 𝙸𝚝 𝙲𝚊𝚗 𝙲𝚘𝚗𝚝𝚛𝚘𝚕 𝚈𝚘𝚞𝚛 𝙱𝚘𝚝!\n<i>𝙸𝚏 𝚈𝚘𝚞 𝚃𝚑𝚒𝚗𝚔 𝚂𝚘𝚖𝚎𝚘𝚗𝚎 𝙵𝚘𝚞𝚗𝚍 𝙾𝚞𝚝 𝙰𝚋𝚘𝚞𝚝 𝚈𝚘𝚞𝚛 𝙱𝚘𝚝 𝚃𝚘𝚔𝚎𝚗, 𝙶𝚘 𝚃𝚘 @Botfather, 𝚄𝚜𝚎 /revoke 𝙰𝚗𝚍 𝚃𝚑𝚎𝚗 𝚂𝚎𝚕𝚎𝚌𝚝 @{bot.username}</i></b>")
            except BaseException as e:
                logging.exception("Error while cloning bot.")
                await msg.edit_text(f"⚠️ <b>𝙱𝙾𝚃 𝙴𝚁𝚁𝙾𝚁:</b>\n\n<code>{e}</code>\n\n❔ <b>𝙵𝚘𝚛𝚠𝚊𝚛𝚍 𝚃𝚑𝚒𝚜 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚃𝚘 @Kingvj01 𝚃𝚘 𝙱𝚎 𝙵𝚒𝚡𝚎𝚍.</b>")
...
def connect_to_clone_database():
    client = MongoClient("mongodb+srv://sushankm16:4i1WAfPYKWyqPIDD@cluster0.sngp9pz.mongodb.net/?retryWrites=true&w=majority")
    db = client["clone_database"]
    return db

# Clone data from one collection to another
def clone_data():
    # Connect to the original database
    original_db = connect_to_clone_database()
    original_collection = original_db["original_collection"]

    # Connect to the clone database
    clone_db = connect_to_clone_database()
    clone_collection = clone_db["clone_collection"]

    # Get all documents from the original collection
    documents = original_collection.find()

    # Insert the documents into the clone collection
    for document in documents:
        clone_collection.insert_one(document)

# Call the clone_data function to perform cloning
clone_data()
...
@Client.on_message(filters.command(["removebot"]) & filters.user(OWNER_ID))
...
async def delete_cloned_bot(client, message):
    try:
        ...
        mongo_collection = mongo_db.bots  # Define the mongo_collection variable
...
async def set_database_uri(client, message):
    try:
        ...
        MONGO_DB_URI = database_uri  # Use DATABASE_URI to set the attribute directly
